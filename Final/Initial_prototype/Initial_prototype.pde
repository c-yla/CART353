...
float breathingGradient = 0.3;
boolean once =true;

boolean startSketch =true;
int monthScaled =0;
...
void setup() {
  ...
  for (int i =0; i< monthData.length; i++)
  {
    ...
    monthData[i].monthScalar =  (int)monthData[i].birthDifference/50;
...
void draw()
{
  ...
  if (startSketch ==true) // Build shape
  {
     ...
     if (state ==0 ) // Birth State.
    {
      ...
      {
        breathingGradient =0;
         //monthScaled =  (int)monthData[currentMonthIndex].birthDifference/50;
